...
def main():
    ...
    model = create_model(
        ...
        # fish:
        NC_mlp_type=NC_mlp_type,
        NC_attn_type=NC_attn_type
        )
    ...
    if not args.s2d:
        ...
        _logger.info('*** Dense Model Created. Processing...  ')

        if args.local_rank == 0:
            _logger.info('Dense Model %s created, param count: %d' %
                        (args.model, sum([m.numel() for m in new_model.parameters()])))
            _logger.info('Dense Model %s created, trainable param count: %d' %
                        (args.model, sum([m.numel() for m in new_model.parameters() if m.requires_grad == True])))

        data_config = resolve_data_config(vars(args), model=new_model, verbose=args.local_rank == 0)

        if args.split_bn:
            assert num_aug_splits > 1 or args.resplit
            new_model = convert_splitbn_model(new_model, max(num_aug_splits, 2))
        ...
        if args.num_gpu > 1:
            if use_amp == 'apex':
                _logger.warning(
                    'Apex AMP does not work well with nn.DataParallel, disabling. Use DDP or Torch AMP.')
                use_amp = None
            new_model = nn.DataParallel(new_model, device_ids=list(range(args.num_gpu))).cuda()
            assert not args.channels_last, "Channels last not supported with DP, use DDP."
        else:
            new_model.cuda()
            if args.channels_last:
                new_model = new_model.to(memory_format=torch.channels_last)

        optimizer = create_optimizer(args, new_model)

        if use_amp == 'apex':
            new_model, optimizer = amp.initialize(new_model, optimizer, opt_level='O1')
        ...
        if S2D_flag:
            _logger.info('New Dense Model Created')
            model = new_model
